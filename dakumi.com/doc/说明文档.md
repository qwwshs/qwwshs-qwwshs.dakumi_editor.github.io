...
- 从左到右的五个轨道分别是```note```轨道 ```x```轨道 ```w```轨道  ```lpos```轨道 ```rpos```轨道
- 以下称事件为```event```

|操作|方法|注意事项|
|----|----|----|
|放置note/wipe/hold|鼠标指针停留在note轨上按下```q```/```w```/```e```键 放置```hold```尾部时需要再次按下```e```|无法重叠放置 且hold尾部beat应该大于hold头部beat|
|放置event|鼠标指针停留在x/w/lpos/rpos轨上按下e键 放置尾部时需要再次按下```e```|无法重叠放置 且event尾部beat应该大于event头部beat|
|删除note/event|鼠标指针停留在note/event上按下```d```键|无|
|选中note/event|左键单击选择|无|
|往复制表里添加/删除该note/event|右键单击选择/删除|无|

::: tip

- event书写规范：event应为两个一组，例如在10beat处放置了一个x event，那么应该在10beat处放置一个w event，以下为规范的，可预测的event的放置情况

![event规范1](/public/event规范1.png)

![event规范2](/public/event规范2.png)

-以下为不可预测，不规范的情况

![event不规范1](/public/event不规范1.png)

![event不规范2](/public/event不规范2.png)
...
|左右翻转event|选中event后按下```alt+b```|镜像事件|
|上下翻转event|选中event后按下```alt+u```|互换起始值和终点值|
|event数值更改|选中event后在play区域按下```alt+t```|将event的值改为鼠标所在的```x```值，如果鼠标所在beat小于event头部的beat更改from，否则更改to|

### play区域

- 点击play区域的轨道会将当前轨道切换至被点击的轨道 如果多个轨道叠在一起 再次点击切换至下一个轨道
...
- 按住左键会出现复制框 在左键松开前按下```shift```会移除复制表中的所有内容 然后添加现在框选的内容
...
- 在该模式中，左键单击选择的event可以在demo区域拖拽起始，终点值，与起始时间，结束时间，如果event的过渡模式为beier，还可以拖动控制点

- 拖拽的起始和终止点会吸附到附件的栅栏中，如果栅栏数量为0,则可以自由移动

- 在该模式中，会禁用点击demo侧轨道切换轨道的功能

- 在该模式中，在demo侧按下鼠标右键会弹出右键菜单，可对当前选中事件进行更改
...
- 如果数值超了范围（除了note_rotate） 超了范围的部分不会显示出来 （比如 note_alpha 值为110 只会显示为100时的效果）
